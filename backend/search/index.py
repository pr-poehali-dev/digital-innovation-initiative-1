...
def _ai_enabled() -> bool:
    ...
    try:
        ...
        if not g or not g[0]:
            return False
        cur.execute(f"SELECT is_enabled FROM {schema}.ai_module_settings WHERE module_code = 'search'")
        m = cur.fetchone()
        ...
        try:
            ...
            cur.execute(
                f"""INSERT INTO {schema}.ai_operation_log
                    ...
                    VALUES ('search', 'YandexGPT', %s, %s, %s, %s)""",
...
def call_yandex_gpt(messages: list) -> str:
    if not _ai_enabled():
        _log_ai_op("chat_with_document", "disabled", data_kind="document_chunks")
        return "[AI-обработка временно отключена владельцем платформы. Включите модуль «Поиск по документам» в разделе «Настройки AI».]"
    ...
    try:
        with urllib.request.urlopen(req, timeout=90) as resp:
            ...
            _log_ai_op("chat_with_document", "success", data_kind="document_chunks", approx_volume=len(payload))
            return result["result"]["alternatives"][0]["message"]["text"]
    except Exception as e:
        _log_ai_op("chat_with_document", "error", data_kind="document_chunks")
