...
ORG_CONTEXT = """КОНТЕКСТ ОРГАНИЗАЦИИ (учитывай при объяснении):
Блок внутреннего контроля банка состоит из четырёх подразделений:
1. Служба внутреннего аудита (СВА) — третья линия защиты, независимая оценка.
2. Департамент по взаимодействию с контрольно-надзорными органами и противодействия
   недобросовестным практикам — работа с регуляторами, противодействие мисселингу
   и недобросовестным практикам.
3. Департамент финансового мониторинга (ДФМ) — ПОД/ФТ, 115-ФЗ, выявление
   подозрительных операций.
4. Департамент внутреннего контроля и комплаенса — вторая линия защиты,
   комплаенс, контрольные процедуры.
Пользователь отвечает за цифровизацию и развитие технологий во всём этом Блоке."""
...
В поле why_matters по возможности привязывай к работе конкретных подразделений Блока.
...
{{
  ...
  "category": "одна из: org, internal_control, digital, ai, budget, management, roles, general"
}}"""
