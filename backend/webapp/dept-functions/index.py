...
def get_or_create_automation(conn, function_id: int, project_id: int) -> dict:
    with conn.cursor() as cur:
        cur.execute(
            f"SELECT id FROM {SCHEMA}.dept_automation WHERE function_id = %s",
            (function_id,),
        )
        row = cur.fetchone()
        if row:
...
Только JSON, без пояснений."""
        ...
        if method == "POST" and action == "ai_recommend":
            ...
            system = """Ты эксперт по цифровой трансформации и автоматизации бизнес-процессов.
Давай конкретные, реалистичные рекомендации с примерами реальных инструментов."""

            prompt = f"""Оцени потенциал автоматизации и дай рекомендации для функции подразделения:
...
{{
  "ai_potential_score": <число от 0 до 10>,
  "ai_recommendation": "<подробный текст 200-300 слов: что автоматизировать, какими инструментами, ожидаемый эффект, приоритет>",
