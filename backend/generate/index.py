...
def build_system_prompt(task: dict, documents: list, web_results: list = None) -> str:
    ...
    # Проверяем: есть ли обязательный шаблон (must_use template)
    mandatory_templates = [d for d in by_role["template"] if d.get("must_use")]
    has_mandatory_template = len(mandatory_templates) > 0
    ...
    if has_mandatory_template:
        tpl_names = ", ".join(d["name"] for d in mandatory_templates)
        parts.append(f"""⚡ РЕЖИМ «ПО ОБРАЗЦУ» АКТИВИРОВАН
Пользователь требует создать презентацию СТРОГО ПО ОБРАЗЦУ: {tpl_names}

ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА В ЭТОМ РЕЖИМЕ:
🔴 1. КОЛИЧЕСТВО СЛАЙДОВ — ровно столько, сколько в образце (±1 если крайне необходимо).
🔴 2. ПОРЯДОК СЛАЙДОВ — в точности как в образце: титульный, оглавление, разделы, итог.
🔴 3. СТРУКТУРА КАЖДОГО СЛАЙДА — заголовок в том же стиле, такое же количество буллетов.
🔴 4. СТИЛЬ ПОДАЧИ — уровень детализации, тон, длина пунктов — как в образце.
🟡 5. СОДЕРЖАНИЕ — наполни своим: используй загруженные материалы и тему задания.
⛔ 6. НЕ придумывай новые разделы, НЕ меняй порядок, НЕ объединяй и НЕ дроби слайды образца.

Документы имеют РОЛИ. Используй их строго по назначению:

1️⃣ TEMPLATE с 🔴 ОБЯЗАТЕЛЬНЫЙ — ВЫСШИЙ ПРИОРИТЕТ В ЭТОМ ЗАДАНИИ.
   Точно воспроизведи его структуру: количество слайдов, их порядок, стиль заголовков, формат буллетов.
   Наполни новым содержанием — но форма должна совпадать с образцом.

2️⃣ STANDARD (стандарты, нормативы) — ВЫСОКИЙ ПРИОРИТЕТ.
   Соблюдай требования стандарта при наполнении содержанием, но НЕ нарушай структуру образца.

3️⃣ CONTENT (содержательные материалы) — ВЫСОКИЙ ПРИОРИТЕТ для содержания.
   Из них берутся ФАКТЫ, тезисы, формулировки для наполнения слайдов образца.

4️⃣ METHODOLOGY (методики) — СРЕДНИЙ ПРИОРИТЕТ.
   Логика аргументации при написании буллетов.

5️⃣ BACKGROUND — фоновый контекст, использовать при необходимости.

ПРАВИЛО: структура и форма — из ОБРАЗЦА. Содержание — из материалов + тема задания.
""")
    else:
...
""")
    ...
    for role_key, role_header in role_blocks:
        ...
        for doc in docs_in_role:
            ...
            if role_key == "template":
                structure = doc.get("structure") or []
                is_mandatory = doc.get("must_use", False)
                if structure:
                    total = len(structure)
                    if is_mandatory:
                        parts.append(f"🔴 ОБЯЗАТЕЛЬНО ВОСПРОИЗВЕСТИ СТРУКТУРУ ТОЧНО ({total} слайдов):")
                        parts.append(f"Создай ровно {total} слайдов в следующем порядке:")
                    else:
                        parts.append(f"Структура образца ({total} слайдов) — возьми форму:")
                    for s in structure[:40]:
                        slide_idx = s.get("index", "?")
                        slide_title = s.get("title", "—")
                        # Показываем первые буллеты чтобы AI понял уровень детализации
                        bullets = s.get("bullets", [])
                        bullet_preview = ""
                        if bullets:
                            sample = [b for b in bullets[:3] if b and len(b.strip()) > 3]
                            if sample:
                                bullet_preview = " | буллеты как: " + "; ".join(sample[:2])
                        parts.append(f"  Слайд {slide_idx}: «{slide_title}»{bullet_preview}")
                    if is_mandatory:
                        parts.append(f"\n⚡ ИТОГО: {total} слайдов. Не добавляй и не убирай слайды без крайней необходимости.")
                # Пример текста для стиля — чуть больше для обязательных шаблонов
                text_limit = 4000 if is_mandatory else 2000
                text_preview = (doc.get("text") or "")[:text_limit]
                if text_preview:
                    label = "Текст образца (стиль заголовков, длина буллетов, тон):" if is_mandatory else "Краткий пример стиля (для тона, НЕ для копирования содержания):"
                    parts.append(f"\n{label}\n{text_preview}")
            else:
    ...
    if has_mandatory_template:
        parts.append("""ПРИ ГЕНЕРАЦИИ (РЕЖИМ «ПО ОБРАЗЦУ»):
1. ПЕРВЫМ ДЕЛОМ: открой структуру образца выше и зафиксируй список всех слайдов.
2. Создай РОВНО столько же слайдов, в том же порядке, с теми же типами заголовков.
3. Каждый слайд наполни НОВЫМ содержанием: используй тему задания + загруженные материалы.
4. Стиль буллетов, длину пунктов, тон — копируй из образца.
5. Помечай источник каждого тезиса в квадратных скобках:
   [из образца: структура]
   [из материалов: <название>]
   [предложено AI]
6. В КОНЦЕ добавь блок «🗺 КАРТА ВЛИЯНИЯ»:
   - Структура → из образца <название>
   - Содержание → из <материалов>
   - Дополнения AI → что придумал сам""")
    else:
