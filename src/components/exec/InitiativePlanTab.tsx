...
export default function InitiativePlanTab({
  ...
  return (
    <div className="space-y-3">
      ...
      {mode === "network" && <DependencyGraphView projectId={planProject.id} />}
...
function PlanMilestonesMode({ projectId }: { projectId: number }) {
  ...
  return <MilestonesTimelineView filters={{ project_id: projectId }} scale="year" dateFrom={range.from} dateTo={range.to} />;
}
...
function PlanTable({ projectId }: { projectId: number }) {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white overflow-hidden">
      {cpm && !cpm.computable && cpm.warnings.length > 0 && (
        <div className="px-3 py-2 bg-amber-50 border-b border-amber-200 text-[11px] text-amber-700">
          {cpm.warnings.join(" ")}
        </div>
      )}
        ...
        <table className="w-full text-sm min-w-[900px]">
          ...
          <tbody className="divide-y divide-slate-100">
            {rows.map((r) => {
              ...
              const isCritical = criticalSet.has(`${r.kind}:${r.item.id}`);
              const isOverdue = isTask ? (r.item as GanttTask).is_overdue : false;
              return (
                <tr key={`${r.kind}-${r.item.id}`} className={isCritical ? "bg-red-50/40" : undefined}>
                  ...
                  <td className="py-2 px-3" style={{ paddingLeft: 12 + r.depth * 20 }}>
                    <span className={`text-sm ${isOverdue ? "text-red-600" : "text-slate-800"}`}>{r.item.title}</span>
                  </td>
