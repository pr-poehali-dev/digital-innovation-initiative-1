...
+                    cur.execute(
+                        "SELECT purpose FROM test_environment_marker "
+                        "LIMIT 1")
+                    row = cur.fetchone()
...
+
+    # Этапы 1 и 2 пройдены обоими независимыми признаками:
+    # именем базы И отдельным маркером назначения. Этап 3
+    # (запись фикстур, HTTP-запросы) разблокируется в фикстурах
+    # ниже — НЕ в pytest_configure — и требует ОТДЕЛЬНОГО
+    # разрешения на применение тестового diff, которого пока нет.
...
| **2** | `SELECT current_database()` **+** отдельная диагностическая таблица `test_environment_marker` с полем `purpose = 'isolated-audit-mode-test'` **+** соединение в режиме `read_only`, где сервер это поддерживает | **Да, только чтение** | Переход к этапу 3 |
...
| 1b | **Локальный маркер тестового пакета** — константа в коде, не из окружения | Код `conftest.py` |
...
| 1d | Флаг `_stage1_passed` явно `False` до прохождения — HTTP и запись невозможны, пока флаг не установлен на этапе 2 | Состояние `pytest` |

**Переменная `TEST_ENV_CONFIRMED` сама по себе недостаточна** —
теперь это один из четырёх независимых признаков этапа 1.

### 7.2. Этап 2 — два независимых признака базы

Проверка одного `current_database()` признана недостаточной.
Добавлен **второй, независимый** признак.

| № | Признак | Что подтверждает |
|---|---|---|
| 2a | Имя базы содержит маркер `test` | Слабый признак — легко подделать именованием |
| 2b | **Таблица `test_environment_marker`** с полем `purpose = 'isolated-audit-mode-test'` | **Сильный признак** — требует, чтобы владелец среды осознанно завёл эту запись при подготовке изолированного контура |

**Оба признака обязательны одновременно.** Совпадения только
имени базы недостаточно.
...
INSERT INTO test_environment_marker (purpose)
VALUES ('isolated-audit-mode-test');
```

Это отдельное требование добавляется в запрос на среду
(`rev2-12`, разд. 7.2, пункт 11).
