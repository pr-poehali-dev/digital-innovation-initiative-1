...
+
+КРИТИЧНО: предохранитель срабатывает в pytest_configure,
+то есть ДО создания любой фикстуры и ДО первого сетевого запроса.
+"""
+import hashlib
+import os
+import uuid
...
+
+# ══════════════════════════════════════════════════════════════
+#  ПРЕДОХРАНИТЕЛЬ — три ОБЯЗАТЕЛЬНЫХ положительных признака
+#  Отсутствие ЛЮБОГО → выход до первого сетевого запроса
+# ══════════════════════════════════════════════════════════════
+def pytest_configure(config):
+    failures = []
+
+    # Признак 1. Явное подтверждение тестовой среды
+    if os.environ.get("TEST_ENV_CONFIRMED") != "isolated-test-env":
+        failures.append(
+            "TEST_ENV_CONFIRMED != 'isolated-test-env'")
+
+    # Признак 2. Проверяемый маркер синтетического проекта
+    if os.environ.get("TEST_PROJECT_ID") != str(SYNTHETIC_PROJECT_ID):
+        failures.append(
+            f"TEST_PROJECT_ID != {SYNTHETIC_PROJECT_ID}")
+
+    # Признак 3. Подтверждение подключения к ТЕСТОВОЙ базе.
+    # Не строковая эвристика: реально подключаемся и спрашиваем
+    # у базы её собственный маркер.
+    dsn = os.environ.get("TEST_DB_DSN", "")
+    if not dsn:
+        failures.append("TEST_DB_DSN не задан")
+    else:
+        try:
...
+                failures.append(
+                    f"База '{dbname}' не помечена как тестовая")
+        except Exception as exc:
+            failures.append(f"Нет подключения к тестовой базе: {exc}")
+
+    if failures:
+        pytest.exit(
+            "ОСТАНОВЛЕНО ДО ПЕРВОГО СЕТЕВОГО ЗАПРОСА.\n"
+            "Не подтверждена изолированная тестовая среда:\n  - "
+            + "\n  - ".join(failures),
+            returncode=2)
+
+
...
## 7. Предохранитель — три обязательных признака

| № | Признак | Способ проверки | Достаточен сам по себе |
|---|---|---|---|
| 1 | `TEST_ENV_CONFIRMED` | Точное совпадение строки | **Нет** |
| 2 | `TEST_PROJECT_ID` | Совпадение с `999001` | **Нет** |
| 3 | Подключение к тестовой базе | **Реальное подключение**, запрос `current_database()`, проверка маркера | **Нет** |

**Отсутствие любого из трёх → выход с кодом 2.**

| Свойство | Реализация |
|---|---|
| Момент срабатывания | `pytest_configure` — **до создания фикстур и до первого HTTP-запроса** |
| Проверка базы | Не строковая эвристика по DSN, а запрос имени базы у самой базы |
| Список запрещённых адресов | Как единственный механизм **не используется** |
| Накопление ошибок | Выводятся все непройденные признаки сразу |
